...
(angle_zone_map). 3D quarter dome.
...
Adopted set is 13 (2026-07-23). Anchor contents: SIDE = the sagittal group,
FRONT = arm slot AND stride angle, OVERHEAD = HSS + pelvis rotational velocity.
Stride angle also sharpens with elevation (el85 r2 0.966, azimuth-independent),
so the overhead band is not exclusively the two transverse metrics -- the ground
...
def main():
    ...
    try:
        ...
        pass
    out = os.path.join(config.OBP_VALIDATION_DIR, "fig_camera_setup_3d.png")
